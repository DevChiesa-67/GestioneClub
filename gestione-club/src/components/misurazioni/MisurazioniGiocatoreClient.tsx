...
export default function MisurazioniGiocatoreClient({
  ...
  const [dolorePresente, setDolorePresente] =
    useState(false);
  ...
  const ultimaAntropometria =
    antropometria.length > 0 ? antropometria[0] : null;

  const ultimoBenessere =
    postAllenamento.length > 0
      ? postAllenamento[0]
      : null;

  const mediaRecupero = useMemo(() => {
    if (postAllenamento.length === 0) {
      return null;
    }

    const totale = postAllenamento.reduce(
      (somma, elemento) =>
        somma + elemento.qualita_recupero,
      0,
    );

    return totale / postAllenamento.length;
  }, [postAllenamento]);
          ...
          value={
            ultimoBenessere
              ? new Intl.DateTimeFormat("it-IT", {
                  ...
                  new Date(
                    `${ultimoBenessere.data_compilazione}T12:00:00`,
                  ),
                )
              : "—"
          }
          ...
          {postAllenamento.map((misurazione) => (
            ...
            >
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                <WellnessValue
                  label="Fatica"
                  value={`${misurazione.fatica}/10`}
                />

                <WellnessValue
                  label="Dolore"
                  value={`${misurazione.dolore_muscolare}/10`}
                />

                <WellnessValue
                  label="Recupero"
                  value={`${misurazione.qualita_recupero}/10`}
                />

                <WellnessValue
                  label="Umore"
                  value={`${misurazione.umore}/10`}
                />
              </div>

              {(misurazione.qualita_sonno !== null ||
                misurazione.ore_sonno !== null) && (
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <WellnessValue
                    label="Qualità sonno"
                    value={
                      misurazione.qualita_sonno !== null
                        ? `${misurazione.qualita_sonno}/10`
                        : "—"
                    }
                  />
          ...
          <div className="max-h-[94vh] w-full overflow-y-auto rounded-t-3xl border border-zinc-800 bg-zinc-950 sm:max-w-xl sm:rounded-3xl">
            ...
            >
              ...
              <RangeField
                name="fatica"
                label="Fatica"
                description="1 = fresco, 10 = esausto"
                icon={<BatteryMedium className="h-5 w-5" />}
              />

              <RangeField
                name="dolore_muscolare"
                label="Dolore muscolare"
                description="1 = nessun dolore, 10 = dolore molto forte"
                icon={<HeartPulse className="h-5 w-5" />}
              />

              <RangeField
                name="qualita_recupero"
                label="Recupero"
                description="1 = recupero scarso, 10 = recupero ottimo"
                icon={<Activity className="h-5 w-5" />}
              />
              ...
              <RangeField
                name="umore"
                label="Umore"
                description="1 = molto basso, 10 = ottimo"
              ...
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    style={{
                      backgroundColor: dolorePresente
                        ? coloreClub
                        : "#3f3f46",
                    }}
                  >
              ...
              <label className="block">
                ...
                <textarea
                  name="note"
                  rows={4}
                  placeholder="Aggiungi eventuali informazioni..."
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="min-w-12 rounded-xl bg-zinc-900 px-3 py-2 text-center text-sm font-bold text-white">
              {value}
            </span>
          </div>
