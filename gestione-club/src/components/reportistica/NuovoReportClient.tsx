...
import { salvaReportPersonalizzato } from "@/app/(dashboard)/reportistica/nuovo/actions";
...
// Sottoinsieme di Aggregazione accettato dalla card pubblicata nel
// riepilogo Performance (deve coincidere con il check constraint SQL su
// report_personalizzati.aggregazione_catapult).
const AGGREGAZIONI_PUBBLICABILI: Aggregazione[] = [
  "media",
  "somma",
  "min",
  "max",
...
type ReportConfig = {
  ...
  legenda: CampoReport[];
  filtri: FiltroReport[];
  // Pubblicazione nel riepilogo Performance: solo per sezionePerformance
  // "performance", solo admin (la pagina che monta questo componente è
  // già riservata agli admin).
  pubblicato: boolean;
  tipiProfiloVisibili: string[];
};

/* =========================================================
   DATA SOURCES
   "Performance" usa i parametri reali di catapult_data; le altre
   sorgenti restano dati dimostrativi in attesa di essere collegate.
========================================================= */
...
const SORGENTI_DATI: SorgenteDati[] = [
  ...
  },
  {
    id: "performance",
    nome: "Performance",
    descrizione: "Metriche GPS e Catapult (dati reali)",
    campi: PARAMETRI_CATAPULT.map((parametro) => ({
      id: `perf_${parametro.campo}`,
      nome: parametro.campo,
      label: `${parametro.label} (${parametro.categoria})`,
      tipo: "number" as const,
      tabella: "catapult_data",
    })),
  },
  {
    id: "test",
...
type Props = {
  tipiProfilo: { codice: string; nome: string }[];
};

export default function NuovoReportClient({ tipiProfilo }: Props) {
  ...
  function toggleTipoProfiloVisibile(codice: string) {
    setConfig((prev) => ({
      ...prev,
      tipiProfiloVisibili: prev.tipiProfiloVisibili.includes(codice)
        ? prev.tipiProfiloVisibili.filter((item) => item !== codice)
        : [...prev.tipiProfiloVisibili, codice],
    }));
  }
  ...
  async function salvaReport() {
    const destinazionePerformance =
      config.sezionePerformance === "performance";
    const campoKpiSelezionato = config.valori[0] ?? null;

    if (config.pubblicato) {
      if (!destinazionePerformance) {
        showToast({
          type: "error",
          message:
      ...
      if (!campoKpiSelezionato) {
        showToast({
          type: "error",
          message:
            "Trascina un parametro Catapult nei Valori prima di pubblicarlo.",
        });
        return;
      }

      if (
        !AGGREGAZIONI_PUBBLICABILI.includes(
          campoKpiSelezionato.aggregazione,
        )
      ) {
        showToast({
          type: "error",
          message:
            "Per la pubblicazione scegli come aggregazione: media, somma, min, max o ultima sessione.",
        });
        return;
      }

      if (config.tipiProfiloVisibili.length === 0) {
        showToast({
          type: "error",
          message:
    ...
    const result = await salvaReportPersonalizzato({
      ...
      pubblicato: config.pubblicato,
      tipi_profilo_visibili: config.tipiProfiloVisibili,
      campo_catapult:
        config.pubblicato && campoKpiSelezionato
          ? campoKpiSelezionato.nome
          : null,
      aggregazione_catapult:
        config.pubblicato && campoKpiSelezionato
          ? (campoKpiSelezionato.aggregazione as
              | "media"
              | "somma"
              | "min"
              | "max"
    ...
    <div className="flex h-[calc(100vh-80px)] min-h-[720px] overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950 text-white">
          ...
          {sorgentiFiltrate.map((sorgente) => {
            ...
            return (
              ...
              >
                ...
                {aperta && (
                  <div className="ml-4 max-h-64 overflow-y-auto border-l border-zinc-800 pl-2">
      ...
      <aside className="flex w-[330px] shrink-0 flex-col border-l border-zinc-800 bg-zinc-950">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* PUBBLICAZIONE NEL RIEPILOGO PERFORMANCE */}
          ...
          {config.sezionePerformance === "performance" && (
            <section className="border-b border-zinc-800 p-4">
              ...
              <label className="mb-3 flex items-center justify-between gap-3 rounded-lg border border-zinc-800 bg-zinc-900/50 p-3">
                <span>
                  <span className="block text-[11px] font-medium text-zinc-200">
                    Pubblicato
                  </span>
                  <span className="block text-[9px] text-zinc-600">
                    Se attivo, il primo campo in &quot;Valori&quot;
                    compare come card nel riepilogo Performance
                  </span>
                </span>

                <input
                  type="checkbox"
                  checked={config.pubblicato}
                  onChange={(event) =>
                    setConfig((prev) => ({
                      ...prev,
                      pubblicato: event.target.checked,
                    }))
                  }
              ...
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">
                Gruppi che possono vederla
              </p>

              <div className="space-y-1.5">
                {tipiProfilo.length === 0 && (
                  <p className="text-[10px] text-zinc-600">
                    Nessun tipo profilo configurato per il club.
                  </p>
                )}

                {tipiProfilo.map((tipo) => (
                  <label
                    key={tipo.codice}
                    className="flex cursor-pointer items-center gap-2 rounded-md px-1 py-1 text-[11px] text-zinc-300 hover:bg-zinc-900"
                  >
                    <input
                      type="checkbox"
                      checked={config.tipiProfiloVisibili.includes(
                        tipo.codice,
                      )}
                      onChange={() =>
                        toggleTipoProfiloVisibile(tipo.codice)
                      }
              ...
              {config.valori[0] ? (
                <p className="mt-3 text-[9px] leading-4 text-zinc-600">
                  ...
                  {" · "}Aggregazione:{" "}
                  <span className="text-zinc-400">
                    {config.valori[0].aggregazione}
                  </span>
                </p>
              ) : (
                <p className="mt-3 text-[9px] leading-4 text-zinc-600">
                  Trascina un parametro nei &quot;Valori&quot; per
                  poterlo pubblicare.
                </p>
              )}
            </section>
...
/* =========================================================
   REPORT PREVIEW
   Solo anteprima UI per bar/line/pie/table/kpi generici (dati
   dimostrativi). Il valore reale del parametro pubblicato viene
   invece calcolato e mostrato nella card del riepilogo Performance,
   non qui.
========================================================= */
